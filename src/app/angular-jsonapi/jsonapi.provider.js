(function() {
  ...
  function jsonapiProvider ($provide){
    ...
    function jsonapiFactory($$AngularJsonAPICollection, $injector){
      var memory = {};
      ...
      function create (type, links) {
        if(memory[type] === undefined){
          console.error('Can\t add not existing object type: ' + type + '. Use initialize(Model, datas).');
        }
        return memory[type].create(links);
      }
      function get (type, id) {
        if(memory[type] === undefined){
          console.error('Can\t get not existing object type: ' + type + '. Use initialize(Model, datas).');
        }
        return memory[type].get(id);
      }
      function all (type) {
        if(memory[type] === undefined){
          console.error('Can\t get all of not existing object type: ' + type + '. Use initialize(Model, datas).');
        }
